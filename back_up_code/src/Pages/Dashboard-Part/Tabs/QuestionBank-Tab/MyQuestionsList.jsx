...
const MyQuestionsList = ({ interviewQuestionsList, setInterviewQuestionsList, section, questionBankPopupVisibility }) => {
  console.log('section from my question list',section)
  ...
  useEffect(() => {
    ...
    const savedLabel = "interviewQuestions";
    if (savedLabel) {
      setSelectedLabel(savedLabel);
    }
    console.log('jiej',myQuestionsList)
  }, []);
  ...
  useEffect(() => {
    if (myQuestionsList && Object.keys(myQuestionsList).length > 0) {
      ...
      setMyQuestionsList(newMyQuestionsObj)
      setFilteredMyQuestionsList(newMyQuestionsObj);
      setLoading(false);
    }
  }, []);
  // }, [myQuestionsList]);
  ...
  const filterQuestions = ()=>{
    ...
    const filtered = Object.keys(myQuestionsList).reduce((acc,key)=>{
      acc[key]=myQuestionsList[key].filter((question)=>{
        const matchesDifficulty = !selectedDifficultyItemsToFilter.length ||
        ...
        const matchesQuestionType = !selectedQuestionTypeFilterItems.length||
  ...
  useEffect(() => {
    filterQuestions();
  }, [  selectedQuestionTypeFilterItems,setSelectedQuestionTypeFilterItems,selectedDifficultyItemsToFilter,setSelectedDifficultyItemsToFilter, myQuestionsList]);
...
const onChangeCheckboxInQuestionType =(e,indx)=>{
  ...
  const updatedFilters = checked
      ...
      setQuestionTypeFilterArray(prev=>
        prev.map((each,index)=>index===indx ? {...each,isChecked:checked}:each)
      )
}
  ...
  const FilterSection =(closeFilter)=>{
    return(
      ...
      <div className="p-2">
         <div className="flex justify-between items-center cursor-pointer" onClick={()=>setIsFilterByDifficultyOpen(!isFilterByDifficultyOpen)}>
          <h3 className="font-medium">Difficulty Level</h3>
          <button>{isFilterByDifficultyOpen ? <FaAngleUp/> : <FaAngleDown />}</button>
          </div>
          <ul>
          {
            isFilterByDifficultyOpen && (
              ...
              difficultyLevelFilterArray.map((level,index)=>(
                <li key={index} className="flex gap-2 cursor-pointer">
                                            <input checked={level.isChecked}  className="w-4 cursor-pointer" value={level.level.toLowerCase()} id={`difficulty-level-${level.level}`} type="checkbox" onChange={(e)=>onChangeCheckboxInDifficultyLevel(e,index)} />
      ...
      <div className="p-2">
         <div className="flex justify-between items-center cursor-pointer" onClick={()=>setIsQuestionTypeFilterOpen(!isQuestionTypeFilterOpen)}>
          <h3 className="font-medium">Question Type</h3>
          <button>{isQuestionTypeFilterOpen ? <FaAngleUp/> : <FaAngleDown />}</button>
          </div>
          <ul>
          {
            isQuestionTypeFilterOpen && (
              ...
              questionTypeFilterArray.map((type,index)=>(
                <li key={index} className="flex gap-2 cursor-pointer">
                                            <input checked={type.isChecked}  className="w-4 cursor-pointer" value={type.type.toLowerCase()} id={`question-type-${type.type}`} type="checkbox" onChange={(e)=>onChangeCheckboxInQuestionType(e,index)} />
      ...
      <div className="bg-white z-50 w-full px-4 ">
        <div
          className={`relative d-block   ${
            section === "interviewerSection" ? "flex justify-end " : ""
          }`}
        >
          <button
        ...
        >
          ...
          <div className="relative">
            <Popup    
            responsive={true}        
            trigger={<button className="border rounded-md p-2">{filterIsOpen ? <LuFilterX className="text-custom-blue " />:  <FiFilter className="text-custom-blue    " /> }</button>}
            ...
            //  const items = useCallback(()=>ItemsFilterFunction(questionsList),[difficultyLevelFilterArray])
              ...
              selectedLabel === listName && (
                <div key={listName} className="mt-4">
                  <div
                    className={` text-white flex  space-x-8 p-2 text-base justify-between items-center bg-custom-blue  font-semibold pr-5 cursor-pointer ${
                      isOpen[listName] ? "rounded-t-md" : "rounded-md"
                    }`}
                  >
                        ...
                        >
                          <div className="flex justify-between items-center border-b pb-2 mb-2 p-2">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div
                                className={`${
                                  question.isCustom ? "visible" : "invisible"
