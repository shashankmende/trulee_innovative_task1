...
app.get('/schedule-assessment/:id',async(req,res)=>{
  try {
    ...
    // console.log('id',id)
    const scheduledAssessment = await scheduledAssessmentsSchema.find({assessmentId:id})
    ...
    console.log("error in getting scheduled assessment from backed",error.message)
    res.status(500).send({
